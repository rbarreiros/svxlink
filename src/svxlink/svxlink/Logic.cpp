...
void Logic::putCmdOnQueue(void)
{
  ...
  // If TOTP is enabled and we are authenticated, consume the authentication now that a command is being processed.
  if (totp_validator->isEnabled() && totp_validator->isAuthenticated())
  {
    totp_validator->consumeAuthentication();
  }
...
void Logic::dtmfDigitDetectedP(char digit, int duration)
{
  ...
  if (totp_validator->isEnabled())
  {
    ...
    if (!totp_validator->isAuthenticated() && !totp_validator->isCollectingInput())
    {
