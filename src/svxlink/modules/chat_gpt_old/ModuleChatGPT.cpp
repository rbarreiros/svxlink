...
extern "C" {
  Module* module_init(void *dl_handle, Logic *logic, const string& cfg_name)
  {
    ...
    try 
    {
      // Validate parameters before creating module
      if (dl_handle == nullptr || logic == nullptr)
      {
        return nullptr;
      }
      ...
      // Remove the debug cout that was causing the crash
      ModuleChatGPT* module = new ModuleChatGPT(dl_handle, logic, cfg_name);
      return module;
    }
    catch (...)
    {
...
{
  // Remove any cout/cerr statements from constructor to avoid issues
...
{  
  ...
  try 
  {
    ...
    {
      cerr << "*** WARNING: Config variable " << cfgName() << "/API_KEY not set\n";
    ...
    m_timeout_timer->expired.connect(sigc::mem_fun(*this, &ModuleChatGPT::onTimeout));
  ...
  {
    cerr << "*** ERROR in ModuleChatGPT::initialize(): " << e.what() << endl;
    return false;
  }
  catch (...)
  {
    cerr << "*** ERROR: Unknown exception in ModuleChatGPT::initialize()\n";
    return false;
  }
}
...
void ModuleChatGPT::cleanup(void)
{
  ...
  {
    cerr << "*** ERROR: Exception in cleanup()\n";
  }
}
...
bool ModuleChatGPT::getChatResponse(const std::string& user_message, std::string& response)
{
  ...
  response = "This is a test response from ChatGPT. Full API integration is needed for real responses.";
  return true;
}
...
void ModuleChatGPT::saveSamplesToFile(const std::vector<float>& samples, const std::string& filename)
{
