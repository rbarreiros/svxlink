...
int main(int argc, char **argv)
{
  ...
  // 4. If no db.conf found, fall back to file backend with svxserver.conf in usual places
  ...
  {
    // Case 3 & 4: No arguments specified, search for db.conf first, then svxserver.conf
    cout << "No configuration arguments specified, searching in standard locations..." << endl;
    if (!cfg.open())
    {
      // If db.conf search failed, try legacy svxserver.conf locations
