...
@brief   Standalone SvxReflector parrot client – main entry point
...
int main(int argc, const char* argv[])
{
  ...
  const char* section_name = "ParrotClient";
  int         version_flag = 0;
  ...
  struct poptOption options[] = {
    ...
    { "section",   's', POPT_ARG_STRING, &section_name, 0,
      "Config section name (default: ParrotClient)", "SECTION" },
  ...
  if (version_flag)
  {
    cout << "parrotclient v" << SVXLINK_APP_VERSION
         << " (" << PROJECT_VERSION << ")\n";
    return 0;
  }
  ...
  {
    cerr << "*** ERROR: ParrotClient initialization failed\n";
    return 1;
  }

  cout << "ParrotClient started. Press Ctrl+C to exit.\n";
  the_app.exec();
  cout << "ParrotClient shutting down.\n";
